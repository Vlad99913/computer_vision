...
def homography_points(m_points, p_points):
    """
    Obtain postwarp homography using selected points
    :param m_points: list of points in the morphed image (at least 4 points required)
    :param p_points: list of corresponding points after postwarp
    :return: postwarp homography matrix
    """
    m_points = np.asarray(m_points, dtype=np.uint8)
    p_points = np.asarray(p_points, dtype=np.uint8)
